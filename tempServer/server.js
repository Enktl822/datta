...
app.get("/products", (request, response) => {  //frontenad -es get gedeg huselt yavuulna  / front-edn ees huselt orj irehdee get gedeg method-oor / arrow function huselt request-eer orj irj bga , response-oor butsah utga damjina
        ...
        if (err) {
            ...
            fs.writeFile("./data/products.json", JSON.stringify(products), (err) => {
                if (err) {
                    ...
                    response
                        .status(200)
                        .send({ success: true, data: products });
                }
            })
